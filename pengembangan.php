...
<?php
	...
	$kid = $_GET['kid'];
	$query_ek = "SELECT pengembangan_diri.*  
							 FROM pengembangan_diri 
							 	WHERE pengembangan_diri.id_pengembangandiri = '" . $kid . "'";
...
<SCRIPT type="text/javascript" src="tool/dhtmlgoodies_calendar.js"></script>
	<h2><img src="images/table_nilai_pengembangan.png" alt=""/></h2>
	<?php 
	$action = $_GET['action'];
	if(empty($action)){?>
...
<tr> 
 ...
 if(!empty($_GET['nis'])){
  ...
  <?php if($_SESSION[wali_kelas]=="wali_kelas"){?>
      ...
      <a href="media.php?pages=pengembangan&action=tambah&nis=<?php echo $_POST['keyword']?>&semester=<?php echo $_POST[semester]?>" class=""><strong>Input Nilai Pengembangan Diri</strong></a>
      <?php }else{?>
      <a href="media.php?pages=pengembangan&action=tambah&nis=<?php echo $_GET['nis']?>&semester=<?php echo $_POST[semester]?>" class=""><strong>Input Nilai Pengembangan Diri</strong></a>
      <?php } ?>
      </td>
      <?php } ?>
      </tr>
  ...
  <td class="search">
      ...
      <td colspan="6" align="right"><input type="submit" name="cari" value="Cari" /></td>
          ...
          <tr><td width="9%" colspan="6"><a href="cetak_pengembangan_pdf.php?nis=<?php echo $_POST[keyword]?>&kelas=<?php echo $_POST[kelas]?>&tahun=<?php echo $_POST[tahun]?>&semester=<?php echo $_POST[semester]?>" target="_blank">Cetak PDF</a></td></tr>
...
<tr class="tabelsorot">
	<th width="90" class="tabelsorot"><strong>No</strong></th>
    <?php if(($_SESSION[status]!="Siswa")){?>
    <th width="200" class="tabelsorot"><strong>NIS</strong></th>
    <?php } ?>
    <th width="300" class="tabelsorot"><strong>Kelas</strong></th>
    
	<th width="300" class="tabelsorot"><strong>Semester</strong></th>
    <th width="350" class="tabelsorot"><strong>Jenis Kegiatan</strong></th>
    <th width="550" class="tabelsorot"><strong>Keterangan</strong></th>
...
if($_SESSION[status]=="Siswa"){
	$tampil = mysql_query("SELECT tsiswa.Nama, tkelas.Nama_Kelas, pengembangan_diri.* 
							 FROM tsiswa 
							 	INNER JOIN pengembangan_diri ON pengembangan_diri.nis = tsiswa.nis
							 	INNER JOIN tkelas ON tkelas.ID_Kelas = pengembangan_diri.id_kelas
								
								WHERE pengembangan_diri.nis = '" . $_SESSION['nama_user'] . "'
	...
	$tampil = mysql_query("SELECT tsiswa.Nama, tkelas.Nama_Kelas, pengembangan_diri.* 
							 FROM tsiswa 
								...
								WHERE pengembangan_diri.nis = '" . $_GET['nis'] . "'
	...
	$cek = mysql_num_rows($tampil);
			...
			$query='';
		if($_SESSION[status]=="Siswa"){
	$query .= " AND pengembangan_diri.nis = '" . $_SESSION['nama_user'] . "' ";
	}
...
if(!empty($_POST[keyword])){
	$query .= " AND pengembangan_diri.nis = '" . $_POST[keyword] . "' ";	
	}
	if(!empty($_POST[semester])){
	$query .= " AND pengembangan_diri.semester = '" . $_POST[semester] . "' ";								 
	}
	
	if(!empty($_SESSION[id_kelas])){
	$query .= " AND pengembangan_diri.id_kelas = '" . $_SESSION[id_kelas] . "' ";
}
		...
		$tampil = mysql_query("SELECT tsiswa.Nama, tkelas.Nama_Kelas, pengembangan_diri.*  
							 FROM tsiswa 
								...
								INNER JOIN ttahun_ajaran ON ttahun_ajaran.id = tsiswa.tahun_ajaran
							 	INNER JOIN tkelas ON tkelas.ID_Kelas = pengembangan_diri.id_kelas
								where pengembangan_diri.id_pengembangandiri <> '' ". $query)or die(mysql_error());
 ...
 <?php $no = 1 ; ?>
	...
	<tr class="tabelsorot">
        ...
        <td class="tabelsorot" align="left"><?php echo $row_kategori['jenis_kegiatan'] ?></td>
        <td class="tabelsorot" align="left"><?php echo $row_kategori['keterangan'] ?></td>
       
			
            <?php if($_SESSION[wali_kelas]=="wali_kelas"){?>
   <td class="tabelsorot" align="center">
			<a href="media.php?pages=pengembangan&amp;action=ubah&amp;kid=<?php echo $row_kategori['id_pengembangandiri'];?>" title="Edit Data"><img src="images/edit2.png" alt="" /></a>
			<a href="prosespengembangan.php?mod=hapus&amp;kid=<?php echo $row_kategori['id_pengembangandiri'];?>" title="Hapus Data" onClick="return confirm('Apakah Anda Yakin Akan Menghapus Data');"><img src="images/tanda_delete.png" alt="" /></a>		            </td>
            <?php } ?>
            </tr>
	 <?php }  ?>
	</table>
...
$tam = mysql_query("SELECT tsiswa.Nama, tkelas.Nama_Kelas, tsiswa.id_kelas  
	   ...
	   $row = mysql_fetch_array($tam);
?>
		<form action="prosespengembangan.php?mod=tambah" method="post" name="form1" id="form1" enctype="multipart/form-data"  onSubmit="MM_validateForm('nis','','R','nama','','R','tahun_ajaran','','R');return document.MM_returnValue">
<table class="bgform" align="right" width="90%">
        <tr><td colspan="3"><strong>Input Nilai Pengembangan Diri</strong></td></tr>
...
<input value="Submit" type="submit" name="submit" onClick="return confirm('Apakah Anda Yakin Dengan Data Yang Dimasukkan');"> <a href="media.php?pages=pengembangan">Cancel</a>
</div>        
	</div>
			</td>
		</tr>
...
<form action="prosespengembangan.php?mod=ubah" method="post" name="form2" id="form2" enctype="multipart/form-data"  onSubmit="MM_validateForm('nama','','R');return document.MM_returnValue">
		<table class="bgform" align="right" width="90%">
       
       <tr><td colspan="3"><strong>Edit Nilai Pengembangan Diri</strong></td></tr>
        <tr><td colspan="3">&nbsp;</td></tr>
		...
		<tr>
			<td>Jenis Kegiatan</td>
			<td>:</td>
			<td><div class="input text"><input type="text" name="jenis" size="47" value="<?php echo $row_ek['jenis_kegiatan']?>" />
            </div></td>
		</tr>
        ...
        <tr>
			<td>Keterangan</td>
			<td>:</td>
			<td><div class="input text"><textarea name="keterangan" cols="50" rows="5"><?php echo $row_ek['keterangan']?></textarea>
            </div></td>
		</tr>
        <!--tr>
		<td class="" align="">Tahun Ajaran</td>
		...
		<tr>
			<td colspan="3" align="right">
	 <div class="type-button">
<div class="submit"><input type="hidden" name="id_pengembangandiri" value="<?php echo $row_ek['id_pengembangandiri']?>" />
