...
<?php
	...
	$kid = $_GET['kid'];
	$query_ek = "SELECT kompetensi.*, kompetensi.Kode_Mp, tmatapelajaran.Nama_MP  
							 FROM kompetensi 
							 	INNER JOIN tmatapelajaran ON tmatapelajaran.Kode_Mp = kompetensi.Kode_Mp
								WHERE kompetensi.id_kompetensi = '" . $kid . "'";
...
<SCRIPT type="text/javascript" src="tool/dhtmlgoodies_calendar.js"></script>
	<h2><img src="images/table_nilai_kompetensi.png" alt=""/></h2>
	<?php 
	$action = $_GET['action'];
	if(empty($action)){?>
...
<table width="25" height="30" align="left"> 
 ...
 <?php } ?>
  ...
  <?php if($_SESSION[wali_kelas]=="wali_kelas"){?>
      ...
      <a href="media.php?pages=kompetensi&action=tambah&kodemp=<?php echo $_POST['KodeMp']?>&semester=<?php echo $_POST[semester]?>" class=""><strong>Input Nilai Kompetensi</strong></a>
  ...
  <td class="search">
      ...
      <td colspan="6" align="right"><input type="submit" name="cari2" value="Cari" /></td>
              ...
              <tr><td width="9%" colspan="6"><a href="cetak_kompetensi_pdf.php?nis=<?php echo $_POST[nis]?>&kelas=<?php echo $_POST[kelas]?>&tahun=<?php echo $_POST[tahun]?>&semester=<?php echo $_POST[semester]?>" target="_blank">Cetak PDF</a></td></tr>
...
<tr class="tabelsorot">
	...
	<th width="450" class="tabelsorot"><strong>Nama MaPel</strong></th>
	<th width="450" class="tabelsorot"><strong>SMTR</strong></th>
    <th width="450" class="tabelsorot"><strong>Nilai</strong></th>
        <th width="450" class="tabelsorot"><strong>KKM</strong></th>
    <th width="350" class="tabelsorot"><strong>Ketercapaian Kompetensi</strong></th>
...
if($_SESSION[status]=="Siswa"){
	$query .= " AND kompetensi.nis = '" . $_SESSION['nama_user'] . "'";
	}
...
if(!empty($_POST[nis])){
	$query .= " AND kompetensi.nis = '" . $_POST[nis] . "'";	
	}
	if(!empty($_POST[semester])){
	$query .= " AND kompetensi.Smester = '" . $_POST[semester] . "'";								 
	}
		
		$tampil = mysql_query("SELECT tsiswa.Nama, tkelas.Nama_Kelas, kompetensi.*, tnilai.nilai, tmatapelajaran.Nama_MP , tmatapelajaran.kkm
							 FROM tsiswa 
							 INNER JOIN ttahun_ajaran ON ttahun_ajaran.id = tsiswa.tahun_ajaran
							 	
								INNER JOIN kompetensi ON kompetensi.NIS = tsiswa.nis
								INNER JOIN tnilai ON tnilai.Kode_Mp = kompetensi.Kode_Mp
								INNER JOIN tkelas ON tkelas.ID_Kelas = kompetensi.id_kelas
								INNER JOIN tmatapelajaran ON tmatapelajaran.Kode_Mp = kompetensi.Kode_Mp WHERE tnilai.id_kelas = kompetensi.id_kelas 
								and kompetensi.NIS = tnilai.NIS and kompetensi.Smester = tnilai.Smester". $query)or die(mysql_error());
...
<?php $no = 1 ; ?>
	...
	<tr class="tabelsorot">
        ...
        <td class="tabelsorot" align="center"><?php echo $row_kategori['nilai'] ?></td>
        <td class="tabelsorot" align="center"><?php echo $row_kategori['kkm'] ?></td>
        <td class="tabelsorot" align="left"><?php echo $row_kategori['ketercapaian_kompetensi'] ?></td>
       
			
            <?php if($_SESSION[wali_kelas]=="wali_kelas"){?>
      <td class="tabelsorot" align="center">
			<a href="media.php?pages=kompetensi&amp;action=ubah&amp;kid=<?php echo $row_kategori['id_kompetensi'];?>" title="Edit Data"><img src="images/edit2.png" alt="" /></a>
			<a href="proseskompetensi.php?mod=hapus&amp;kid=<?php echo $row_kategori['id_kompetensi'];?>" title="Hapus Data" onClick="return confirm('Apakah Anda Yakin Akan Menghapus Data');"><img src="images/tanda_delete.png" alt="" /></a>
            </td>
            <?php } ?>
            </tr>
...
?>
		<form action="proseskompetensi.php?mod=tambah" method="post" name="form1" id="form1" enctype="multipart/form-data"  onSubmit="MM_validateForm('nilai[1]','','R','nilai[2]','','R','tahun','','R');return document.MM_returnValue">
        <input type="hidden" name="jum" value="<?=$jum?>" />
<table class="bgform" align="right" width="90%">
        <tr>
        <td colspan="3"><strong>Input Nilai Kompetensi</strong></td>
        </tr>
        <?php 
       ...
       <tr><td colspan="5">Nama Mata Pelajaran &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp; <?=$m[Nama_MP]?></td></tr>
        ...
        <tr class="tabelsorot">
        	<td width="150" class="tabelsorot" align="center">NIS</td>
            <td width="250" class="tabelsorot" align="center">Nama</td>
            <td width="200" class="tabelsorot" align="center">Ketercapaian Kompetensi</td>
        </tr>
      ...
      <?php 
		...
		<td> 
        	<input type="text" size="50" name="nilai[<?=$i?>]" />
         </td>   
        </tr>
          <?php $i++; } ?> 
...
<tr><td colspan="3">&nbsp;</td></tr>
       ...
       <tr><td colspan="3">Siswa ini Sudah Ada Nilai Kompetensi Pada Semester yang Anda Pilih ! Silahkan Pilih Semester Yang Lain</td></tr>
       <?php endif;?>
		<tr>
...
<input value="Submit" type="submit" name="submit" onClick="return confirm('Apakah Anda Yakin Dengan Data Yang Dimasukkan');"> <a href="media.php?pages=kompetensi">Cancel</a>
</div>        
	</div>
			</td>
		</tr>
...
<form action="proseskompetensi.php?mod=ubah" method="post" name="form2" id="form2" enctype="multipart/form-data"  onSubmit="MM_validateForm('nama','','R');return document.MM_returnValue">
		<table class="bgform" align="right" width="90%">
       
       <tr><td colspan="3"><strong>Edit Nilai Kompetensi</strong></td></tr>
        <tr><td colspan="3">&nbsp;</td></tr>
        ...
        <tr>
			<td>Ketercapaian Kompetensi</td>
			<td>:</td>
			<td><div class="input text"><input type="text" name="nilai" size="45" value="<?php echo $row_ek['ketercapaian_kompetensi']?>" /></td>
		</tr>
		...
		<tr>
			<td colspan="3" align="right">
	 <div class="type-button">
<div class="submit"><input type="hidden" name="id_kompetensi" value="<?php echo $row_ek['id_kompetensi']?>" />
