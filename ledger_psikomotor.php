...
<html lang="en">
<head>
    <title>LEDGER NILAI PSIKOMOTOR</title>
</head>
<body>
<?php 
...
<div class="line"></div>
        <table align="center">
<tr>
        <td align="center"><strong>Ledger Daftar Nilai Psikomotor</strong></td>
...
$cek = mysql_num_rows($sql_siswa);
                   ...
                   <tr>
					...
					while($p=mysql_fetch_array($sql_r)){
						...
						//print_r($rs['nilai']);
						$rata = $rata + $rs['praktik'];
					?>
                    <td align="center">&nbsp;<?php echo $rs['praktik']?>&nbsp;</td>
                    <?php } ?>
...
$rata=0;
						...
						$n = mysql_query("SELECT SUM(praktik) as nilai FROM tnilai WHERE tnilai.Kode_Mp='$row[Kode_Mp]' and ID_Kelas='$id_kelas'")or die(mysql_error());
...
$rata=0;
						while($row = mysql_fetch_array($sql_r)){
						$n = mysql_query("SELECT Max(praktik) as nilai FROM tnilai WHERE tnilai.Kode_Mp='$row[Kode_Mp]' and ID_Kelas='$id_kelas'")or die(mysql_error());
						$rs2 = mysql_fetch_array($n);
						if($rs2['nilai']!=0){
					?>
...
$rata=0;
						while($row = mysql_fetch_array($sql_r)){
						$n = mysql_query("SELECT Min(praktik) as nilai FROM tnilai WHERE tnilai.Kode_Mp='$row[Kode_Mp]' and ID_Kelas='$id_kelas'")or die(mysql_error());
						$rs2 = mysql_fetch_array($n);
						if($rs2['nilai']!=0){
					?>
                    <td align="center">&nbsp;<?php echo $rs2['nilai']?>&nbsp;</td>
