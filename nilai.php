...
<?php
	...
	$kid = $_GET['kid'];
	$query_ek = "SELECT tnilai.*, tnilai.Kode_Mp, tmatapelajaran.Nama_MP, tsiswa.Nama  
							 FROM tnilai 
								INNER JOIN tsiswa ON tsiswa.nis = tnilai.nis							 
							 	INNER JOIN tmatapelajaran ON tmatapelajaran.Kode_Mp = tnilai.Kode_Mp
								WHERE tnilai.ID_Nilai = '" . $kid . "'";
...
<SCRIPT type="text/javascript" src="tool/dhtmlgoodies_calendar.js"></script>
	<h2><img src="images/table_nilai.png" alt=""/></h2>
	<?php 
	//print_r($_SESSION);
	$action = $_GET['action'];
	if(empty($action)){?>
...
<table width="1007" cellpadding="0" cellspacing="0">
 ...
 <tr>
  <td class="search">
    ...
    <tr> 
      ...
      <td width="90%"><!--<input type="text" name="keyword" value="<? echo $_POST[keyword]; ?>">-->
      <select name="KodeMp">
      ...
      <tr>      
      <td colspan="3" align="right"><input type="submit" name="filter" value="Submit"></td>
    </tr>
 ...
 if(isset($_POST['filter'])){
	   //print_r($_GET['nis']);
	   $tam = mysql_query("SELECT tmatapelajaran.* from tmatapelajaran where Kode_Mp='$_POST[KodeMp]' 							 
							 ")or die(mysql_error());
	//   }
...
<table width="25" height="30" align="left"> 
 ...
 <?php } ?>
  ...
  <?php if($_SESSION[wali_kelas]=="wali_kelas"){?>
      ...
      <a href="media.php?pages=nilai&action=tambah&kodemp=<?php echo $_POST['KodeMp']?>&semester=<?php echo $_POST[semester]?>" class=""><strong>Input Nilai</strong></a>
      <?php } ?>
      </td>
      <?php } ?>
      </tr>
  ...
  <td class="search">
   ...
   <tr> 
      ...
      <td colspan="4" width="26%"><!--<input type="text" name="kelas" value="<? echo $_POST[kelas]; ?>">-->
      
       <select name="kelas">
				<option value="">- Pilih Kelas -</option>
                <?php 
					$sql = mysql_query("SELECT tkelas.* FROM tkelas ORDER BY Nama_Kelas")or die(mysql_error());
					while($guru = mysql_fetch_array($sql)){
				?>
       ...
       <tr> 
      <td width="25%">Cari Nama Mata Pelajaran :</td>
      
      <td colspan="4" width="26%"><!--<input type="text" name="mapel" value="<? echo $_POST[mapel]; ?>">-->
      <select name="mapel">
				<option value="">- Pilih Nama Mata Pelajaran -</option>
                <?php 
					$sql = mysql_query("SELECT tmatapelajaran.*, tguru.Nama_Guru FROM tmatapelajaran , tguru where tguru.NIP = tmatapelajaran.id_guru ORDER BY Kode_Mp")or die(mysql_error());
					while($guru = mysql_fetch_array($sql)){
				?>
				<option value="<?php echo $guru['Kode_Mp']?>"><?php echo $guru['Nama_MP']. " (". $guru['Nama_Guru']?>)</option>
                <?php } ?>
  ...
  <tr>
  	<td colspan="6" style="font-size:10px; color:#000">*) Untuk Melihat Nilai Maximal & Minimal, Silahkan Cari Berdasarkan Kelas & Nama Mata Pelajaran</td>
  </tr>
  <tr><td width="9%" colspan="6"><a href="cetak_nilai_pdf.php?nis=<?php echo $_POST[nis]?>&kelas=<?php echo $_POST[kelas]?>&tahun=<?php echo $_POST[tahun]?>&semester=<?php echo $_POST[semester]?>&mapel=<?php echo $_POST[mapel]?>" target="_blank">Cetak PDF</a></td>
  </tr>
...
?>
  ...
  <?php if($_GET['pesan']=="ada"):?>
       <tr><td colspan="5">Sudah Ada Nilai Pada Semester dan Mata Pelajaran Yang Anda Pilih ! </td></tr>
       <?php endif;?>
  </table>
    </form>   
  </td>
  ...
  <?php 
  	/*if(!empty($_POST[kelas]) && !empty($_POST[mapel])){
		$n1 = mysql_query("SELECT max(nilai) as nilai_max from tnilai where id_kelas='$_POST[kelas]' and Kode_Mp='$_POST[mapel]'")or die(mysql_error());						
$sa = mysql_fetch_array($n1);
 $nilai_max = $sa['nilai_max'];
 $nn1 = mysql_query("SELECT NIS, Kode_Mp, id_kelas from tnilai where nilai=$nilai_max")or die(mysql_error());
 $ambil_nis = mysql_fetch_array($nn1);
 
 mysql_query("UPDATE tnilai SET keterangan='' 
 WHERE keterangan='Nilai Maximal' and nis <> '$ambil[NIS]'  and id_kelas='$row_default[id_kelas]'")or die(mysql_error());

 mysql_query("UPDATE tnilai SET keterangan='Nilai Maximal' WHERE nis='$ambil_nis[NIS]' and id_kelas='$row_default[id_kelas]'");
		}*/
  ?>
<tr>
    <td class="bgform">
...
<tr class="tabelsorot">
    ...
    <th width="200" class="tabelsorot"><strong>NIS</strong></th>
 	...
 	<?php } ?>
	<th width="150" class="tabelsorot"><strong>Kelas</strong></th>
	<th width="250" class="tabelsorot"><strong>Nama Mata Pelajaran</strong></th>
	<th width="100" class="tabelsorot"><strong>Semester</strong></th>
    <th width="100" class="tabelsorot"><strong>Nilai</strong></th>
    <th width="100" class="tabelsorot"><strong>KKM</strong></th>
    <th width="200" class="tabelsorot"><strong>Keterangan</strong></th>
    <th width="50" class="tabelsorot" align="center"><strong>Actions</strong></th>
...
if(!empty($_SESSION[id_kelas])){
	$query .= " AND tnilai.id_kelas = '" . $_SESSION[id_kelas] . "' ";
}

if($_POST[cari]=="Cari"){
	
	if(!empty($_POST[kelas])){
		  $query.= " AND tnilai.id_kelas = '" . $_POST[kelas] . "' ";
    }
   
    if(!empty($_POST[semester])){ 
		$query .= " AND tnilai.Smester = '" . $_POST[semester] . "' ";								 
	}
	
	if(!empty($_POST[nis])){
		$query .= " AND tsiswa.nis = '" . $_POST[nis] . "' ";	
	}
	...
	if(!empty($_POST[mapel])){
			//print_r($_POST);
			$query .= " AND tnilai.Kode_Mp ='" . $_POST[mapel] . "' ";
			}
}
...
$sa = mysql_fetch_array($n1);
 ...
 $n2=mysql_query("SELECT min(nilai) as nilai_min from tnilai  where id_kelas='$_POST[kelas]' and Kode_Mp='$_POST[mapel]'")or die(mysql_error());
 $si = mysql_fetch_array($n2);
 $nilai_min = $si['nilai_min'];
	  
	 // var_dump("SELECT max(nilai) as nilai_max from tnilai where id_kelas='$_POST[kelas]' and Kode_Mp='$_POST[mapel]'");
	  //echo $nilai_max;
	?>	
 <?php $no = 1 ; ?>
 <?php 
 $tampil_default = mysql_query("SELECT tsiswa.Nama, tkelas.Nama_Kelas, tnilai.*, tmatapelajaran.*
   
							 FROM tsiswa 
							 	
								INNER JOIN ttahun_ajaran ON ttahun_ajaran.id = tsiswa.tahun_ajaran
								INNER JOIN tnilai ON tnilai.NIS = tsiswa.nis
								INNER JOIN tkelas ON tkelas.ID_Kelas = tnilai.id_kelas
								
								
								INNER JOIN tmatapelajaran ON tmatapelajaran.Kode_Mp = tnilai.Kode_Mp WHERE tnilai.ID_Nilai <> '' " . $query)or die(mysql_error());

							
 ?>
 <?php while ($row_default = mysql_fetch_assoc($tampil_default)){ 
 if($nilai_max == $row_default['nilai']){
	 $n = "Nilai Maksimum";
	 }elseif($nilai_min == $row_default['nilai']){
		 $n = "Nilai Minimum";
		 }else{
			 $n="";
			 }
 ?>
	...
	<tr class="tabelsorot">
        ...
        <?php if(($_SESSION[status]!="Siswa")){?>
        <td class="tabelsorot"><?php echo $row_default['NIS']?></td>
        
        <?php } ?>
		<td class="tabelsorot" align="left"><?php echo $row_default['Nama_Kelas']?></td>
		<td class="tabelsorot" align="left"><?php echo $row_default['Nama_MP']?></td>
		<td class="tabelsorot" align="left"><?php echo $row_default['Smester'] ?></td>
        <td class="tabelsorot" align="center"><?php echo $row_default['nilai'] ?></td>
        <td class="tabelsorot" align="center"><?php echo $row_default['kkm'] ?></td>
        <td class="tabelsorot" align="left"><?php echo $n ?></td>
        
    

         <td width="157" class="tabelsorot" align="center">
        	<a href="media.php?pages=nilai&amp;action=view&amp;kid=<?php echo $row_default['ID_Nilai'];?>" title="Lihat Data Lengkap"><img src="images/tanda_view.png" alt="" /></a>
		 <?php if($_SESSION[wali_kelas]=="wali_kelas"){?>
			<a href="media.php?pages=nilai&amp;action=ubah&amp;kid=<?php echo $row_default['ID_Nilai'];?>" title="Edit Data"><img src="images/edit2.png" alt="" /></a>
			<a href="prosesnilai.php?mod=hapus&amp;kid=<?php echo $row_default['ID_Nilai'];?>&n=<?=$nis?>" title="Hapus Data" onClick="return confirm('Apakah Anda Yakin Akan Menghapus Data');"><img src="images/tanda_delete.png" alt="" /></a>
            </td>
            <?php } ?>
            </tr>
	 <?php }  ?>
...
?>
		<form action="prosesnilai.php?mod=tambah" method="post" name="form1" id="form1" enctype="multipart/form-data"  onSubmit="MM_validateForm('nilai[1]','','RisNum','praktik[1]','','RisNum');return document.MM_returnValue">
        <input type="hidden" name="jum" value="<?=$jum?>" />
<table class="bgform" align="right" width="90%">
       <tr><td colspan="5"><strong>Input Nilai</strong></td></tr>
        <tr><td colspan="5">&nbsp;</td></tr>
       <?php 
	   $sql = mysql_query("SELECT * FROM tmatapelajaran WHERE Kode_Mp='$_GET[kodemp]'")or die(mysql_error());
		$m = mysql_fetch_array($sql);
	   ?>
       <tr><td colspan="5">Nama Mata Pelajaran &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp; <?=$m[Nama_MP]?></td></tr>
        <tr><td colspan="5">Semester &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp; <?=$_GET[semester]?></td></tr>
        ...
        <tr class="tabelsorot">
            ...
            <td width="10" class="tabelsorot" align="center">Nilai</td>
            <td width="10" class="tabelsorot" align="center">Praktik</td>
            <td width="10" class="tabelsorot" align="center">Sikap</td>
        </tr>
        <?php 
		$i=1;
		...
		<td align="center"> 
        	<input type="text" size="2" name="nilai[<?=$i?>]" maxlength="3" />
         </td>   
        
        <td align="center">
        <input type="text" size="2" name="praktik[<?=$i?>]"  maxlength="3" />
        </td>
            
<td align="center"><input type="text" size="2" name="predikat[<?=$i?>]"  maxlength="1" />
</td>
          </tr>
          <?php $i++; } ?>      
...
<input value="Submit" type="submit" name="submit" onClick="return confirm('Apakah Anda Yakin Dengan Data Yang Dimasukkan');"> <a href="media.php?pages=nilai">Cancel</a>
</div>        
	</div>
			</td>
		</tr>
        ...
        <tr>
			<td colspan="5" align="right">*) Data Harus Di Isi Dengan Benar !
            </td>
		</tr>
...
<form action="prosesnilai.php?mod=ubah" method="post" name="form2" id="form2" enctype="multipart/form-data"  onSubmit="MM_validateForm('nama','','R');return document.MM_returnValue">
		<table class="bgform" align="right" width="90%">
        
       <tr><td colspan="3"><strong>Edit Nilai</strong></td></tr>
		<tr><td colspan="3">&nbsp;</td></tr>
        
        <!-- <tr>
		<td class="" align="">Mata Pelajaran</td>
        <td>:</td>
		<td class="" align=""><div class="input text">
		    ...
		    <tr>
		<td class="" align="">Nama Mata Pelajaran</td>
        <td>:</td>
		<td class="" align=""><div class="input text">
		    <select name="Kode_Mp">
		        <option value="">- Pilih Nama Mata Pelajaran -</option>
        ...
        <tr>
			<td>Nilai</td>
			<td>:</td>
			<td><div class="input text"><input type="text" size="2" name="nilai" value="<?php echo $row_ek['nilai']?>" maxlength="3" /></td></tr>
            <td>Praktik</td>
			<td>:</td>
			<td><div class="input text"><input type="text" size="2" name="praktik" value="<?php echo $row_ek['praktik']?>" maxlength="3" /></td></tr>
            
            <td>Predikat</td>
			<td>:</td>
			<td><div class="input text"><input type="text" size="2" name="predikat" value="<?php echo $row_ek['predikat']?>"  maxlength="1" /></td></tr>
            </div></td>
		</tr>
	 ...
	 <div class="type-button">
<div class="submit"><input type="hidden" name="ID_Nilai" value="<?php echo $row_ek['ID_Nilai']?>" />
...
<?php }elseif($action=="view"){ ?>
       ...
       <tr><td colspan="3"><strong>Lihat Nilai</strong></td></tr>
		<tr><td colspan="3">&nbsp;</td></tr>
        <tr>
			<td width="30%">NIS</td>
			<td>:</td>
        ...
        <tr>
			...
			<td>:</td>
			<td width="70%"><?php echo $row_ek['nilai']?></td>
		</tr>
                <tr>
			<td width="30%">Praktik</td>
			<td>:</td>
			<td width="70%"><?php echo $row_ek['praktik']?></td>
		</tr>
		        <tr>
			<td width="30%">Predikat</td>
			<td>:</td>
			<td width="70%"><?php echo $row_ek['predikat']?></td>
		</tr>
	 ...
	 <div class="type-button">
<div class="submit">
<a href="media.php?pages=nilai"><strong>Kembali</strong></a>
</div>        
	</div>
			</td>
		</tr>
